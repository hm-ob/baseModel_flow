...
class TorchTrainer(ABC, nn.Module):
    ...
    def train_epoch(
        ...
        for key, value in epoch_log.items():
            ...
            mlflow.log_metric(f"epoch_train_{key}", np.mean(value), step=step)

        if eval_data is not None:
            self.network.eval()
            logger.info("Performing evaluation...")
            if hasattr(self, "test_epoch_start"):
            ...
            with torch.no_grad():
                ...
                if hasattr(self, "test_epoch_end"):
                    epoch_metrics = self.test_epoch_end()
    ...
    def evaluate(
        ...
        if hasattr(self, "test_epoch_end"):
            ...
            for k, v in epoch_metrics.items():
                ...
                mlflow.log_metric(f"test_{k}", v, step=self.global_step)
    ...
    def load_weights(self, path: str, device: str = "cuda"):
        ...
        Args:
            path (str): Path to the checkpoint file.
            device (str, optional): Device to map the weights to. Defaults to "cuda".
        """
    ...
    def load_all_states(self, path: str, device: str = "cuda"):
    ...
    def compile(
        ...
        if type(optimizer) == str:
            available_optimizers = {
                "sgd": optimizers.sgd(
                    self.network.parameters(), lr=0.01, momentum=0.9
                ),
                "adam": optimizers.adam(self.network.parameters(), learning_rate=0.001),
                ...
                "adamw": optimizers.adamw(
                    self.network.parameters(), learning_rate=0.001, weight_decay=0.01
                ),
            }
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            ...
            For example, {"input_1": inputs_1, "input_2": inputs_2} or [inputs_1, inputs_2].
        Returns:
